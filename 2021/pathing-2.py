...
class cave:
    def __init__(self, cave_info):
        ...
        self.visited_one_small_cave_twice = False
        ...
        for node in self.graph[start]:
            if node.islower():
                if (node not in path):
                    ...
                    small_caves_used = [node for node in path if node.islower() and node != 'start' and node != 'end']
                    small_caves_used_filtered = set(small_caves_used)
                    if (len(small_caves_used_filtered) == len(small_caves_used)):
                        newpaths = self.find_all_paths(node, end, path)
                        for newpath in newpaths:
                            paths.append(newpath)
