...
def probe_on_target(pos,x_range,y_range):
    x_pos_check = pos[0] in range(x_range[0],x_range[1]+1)
    y_pos_check = pos[1] in range(y_range[0],y_range[1]+1)
...
def probe_beyond_target(pos,vel,x_range,y_range):
...
#        target_y_max = temp_y
    ...
    for x in range(0,300):
        for y in range(target_y_min,300):
            ...
            steps = 0
            while (continue_stepping == True):
#                print ("starting new loop","{",x,",",y,"}",curr_pos,curr_vel)
...
#                    print ("Target hit!")
                    ...
                    beyond_target = probe_beyond_target(curr_pos,curr_vel,target_x,target_y)
                    if (beyond_target == True):
                        continue_stepping = False
#                        print ("We went too far! Pos:",curr_pos,"Target:",target_x,target_y,continue_stepping)
