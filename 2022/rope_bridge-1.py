...
def process_instruction(line):
    ...
    if (direction == "R"):
        while (move_dist > 0):
            ...
            if (get_dist_between_locs(H_loc, T_loc) > 1.5):
                #print ("moving after R")
                H_loc,T_loc = move_tail(H_loc,T_loc)
                T_locs.add((T_loc[0],T_loc[1]))
            move_dist -= 1
    ...
    if (direction == "D"):
        while (move_dist > 0):
            H_loc[0] += 1
            if (get_dist_between_locs(H_loc, T_loc) > 1.5):
                #print ("moving after D")
                H_loc,T_loc = move_tail(H_loc,T_loc)
                T_locs.add((T_loc[0],T_loc[1]))
            move_dist -= 1

    H_curr_loc.append(H_loc)
    T_curr_loc.append(T_loc)
...
def main():
    ...
    #filename = "aoc_2022_input_9-test.txt"
